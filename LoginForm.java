...
public class LoginForm extends TransparentPanel
{
  ...
  private TransparentButton retryButton = new TransparentButton("Ещё раз");
  private TransparentButton offlineButton = new TransparentButton("Играть офлайн");
  ...
  private JPanel buildLoginPanel() {
    ...
    try
    {
      ...
      {
        TransparentLabel accountLink = new TransparentLabel("Быстрая регистрация") {
        ...
        accountLink.addMouseListener(new MouseAdapter() {
          public void mousePressed(MouseEvent arg0) {
            try {
              Util.openLink(new URL("https://authserver.ely.by/register").toURI());
            } catch (Exception e) {
              e.printStackTrace();
            }
          }
  ...
  private TransparentLabel getUpdateLink() {
    TransparentLabel accountLink = new TransparentLabel("           Забыли пароль?") {
      private static final long serialVersionUID = 0L;
  ...
  private TransparentPanel buildOfflinePanel()
  {
    ...
    if (!canPlayOffline) {
      loginPanel.add(new TransparentLabel("(нет кэша)", 4), "South");
    }
  ...
  public void setLoggingIn() {
    ...
    southPanel.add(new LogoPanel(), "West");
    southPanel.add(new TransparentPanel(), "Center");
    JLabel label = new TransparentLabel("Вход в систему...                      ", 0);
